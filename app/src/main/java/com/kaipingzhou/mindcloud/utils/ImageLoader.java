...
/**
 * Created by 周开平 on 2017/3/27 20:53.
 * qq 275557625@qq.com
 * 作用：
 */
    ...
    public static Bitmap decode(final String pfad, int width, int height, final float[] imgDetails) {
        ...
        try {
            ...
            if (rotation > 0) {
                ...
                if (true)
                    Logger.log("rotate image by " + rotation);
                Bitmap image = BitmapFactory.decodeFile(pfad, options);
                try {
                    return Bitmap.createBitmap(image, 0, 0, imgWidth / options.inSampleSize, imgHeight / options.inSampleSize, matrix, true);
                } catch (Exception e) {
                    e.printStackTrace();
                    return null;
                }
